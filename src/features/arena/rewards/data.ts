...
export async function getArenaSummary(): Promise<Result<ArenaSummaryRead>> {
  const supabase = await createServerClient();
  const result = await fromSupabase<unknown>(async () => {
    // ⚠️ The cast is a workaround, not a shortcut. `database.types.ts` is
    // generated by `npm run db:types`, which shells out to `supabase gen types`
    // and needs a Docker daemon this machine does not have (WS-8 recorded the
    // same wall). The file is also WS-0's, and WS-11 may not edit it — so the
    // new RPC is unknown to the generated union and the call will not compile
    // without this. Filed as ISSUES I-052 for WS-13 to regenerate.
    const { data, error } = await (
      supabase.rpc as unknown as (
        name: string,
      ) => Promise<{ data: unknown; error: import("@supabase/supabase-js").PostgrestError | null }>
