-- ---------------------------------------------------------------------------
-- duplicate_course, corrected: content_versions.snapshot is NOT NULL.
--
-- The first cut inserted the new version with `snapshot = null`, reasoning that
-- a draft has nothing to freeze yet and publishing would fill it. The column
-- rejects that, and rightly: `content_versions_snapshot_check` requires a json
-- object, so "no snapshot" has to be spelled `{}` rather than absent.
--
-- Copying the source row's snapshot would have satisfied the constraint and
-- been worse. A snapshot is a frozen render of one version's *ids*; pasting it
-- onto the copy would hand every learner the original course's stage and task
-- ids, and the copy would serve the original's content until someone
-- republished it.
--
-- So the version is inserted with `{}` and the snapshot is built at the end,
-- from the rows this function has just created, with the same
-- `app_private.build_content_snapshot` that publishing uses. The build has to
-- come last: it reads stages and tasks, and those do not exist until the clone
-- has walked the whole subtree.
-- ---------------------------------------------------------------------------
