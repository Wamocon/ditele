-- ---------------------------------------------------------------------------
-- duplicate_course — copy a course and everything that IS the course.
--
-- What it copies, and just as importantly what it refuses to.
--
-- The foreign-key graph under `courses` mixes two very different things:
-- authored content, and the record of real people doing it. `enrollments`,
-- `cohorts`, `attempts`, `submissions`, `evidence`, `questions` and
-- `certificates` all hang off a course or its tasks. Copying those would
-- silently enrol live learners into a course nobody has published, and mint
-- attempt rows they never made. So the clone walks the content subtree only:
--
--   course → course_localizations
--          → content_versions (latest only) → stages → stage_localizations
...
--                                                    → task_hints
--                                                    → task_skill_mappings
--                                           → prerequisites (both ends remapped)
--
-- `task_schedules` is NOT copied: it is keyed by cohort, so it belongs to a
-- group running the course, not to the course itself.
--
-- `course_trainers` is deliberately NOT copied. A duplicate is a new course; who
-- teaches it is a decision, not a property of the content.
--
-- Only the LATEST content version is copied, and it lands as version 1 in
-- `draft`. Copying a published version would produce a course that claims to be
-- live the moment it exists, and copying every historical version would produce
-- a version history that never happened. `snapshot` is set to null on purpose:
-- it is a frozen render of the old course's ids, and publishing rebuilds it.
--
-- `external_id` is nulled everywhere it exists. It is unique per source_system
-- and identifies a row in a system upstream of this one; two rows claiming the
-- same external identity is a data-integrity bug waiting for an import to run.
--
-- Ordering matters and is why this is one function rather than several calls:
-- prerequisites reference two tasks, so every task must exist before any
-- prerequisite is remapped. Doing it in the app would mean a half-copied course
...
begin
  ...
  values (
    ...
    source_course.source_system, null, v_actor_id
  )
  ...
  values (new_course_id, 1, 'draft',
          'Kopie von ' || source_course.slug, null, v_actor_id)
  ...
  with inserted as (
    ...
    select new_course_id, new_version_id, s.position, s.state, s.source_system, null
  ...
  with inserted as (
    ...
    select new_course_id, m.new_id, new_version_id, t.bug_category_id, t.position, t.task_kind,
           ...
           t.source_system, null, t.video_url, t.intro_video_url, t.document_url
    from public.tasks t
